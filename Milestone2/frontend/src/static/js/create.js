...
function isCreatedAlready(id) {
  let found = currentUserBet.find(bet => bet.game == id);
...
function createWager(game, amount) {
  api.postBet({
    ...
    "user2": null,
    "team": getWhichTeamPicked(),
    "status": 1, // pending
    "amount": amount
  });
}

function reduceBalanceBy(amount) {
  console.log(currentUser.credits - amount);
  api.changeBalance(currentUser.id, currentUser.credits - amount).then(() => {
    api.getCurrentUser().then(user => currentUser = user);
  });
